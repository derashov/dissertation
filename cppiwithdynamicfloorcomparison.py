# -*- coding: utf-8 -*-
"""
Created on Sun Feb 26 20:40:03 2023
...
def run_sim_cir(n_years=10, n_scenarios=1000, a=1, b=0.004, rt=0.1, sigma=0.09, steps_per_year=12):
    ...
    return output
def run_cppi_cir_d(risky_r, safe_r, m=3, start=1000, floor=0.8, a = 1, b = 0.1/12, rt = 0.1/12, sigma = 0.09, drawdown = 0.1):
    """
    ...
    backtest_result = {
        ...
        "drawdown": drawdown,
        "peak": peak_history,
        "floorhist": floorval_history
    }
    return backtest_result

def show_cppi_cir_d(n_scenarios = 50, mu = 0.07, sigma1 = 0.15, m = 3, floor = 0.8, a = 1, b = 0.1/12, rt = 0.1/12, sigma = 0.09, drawdown = 0.1, y_max = 100):
    start = 100
    ...
    btr = run_cppi_cir_d(risky_r = pd.DataFrame(risky_r), safe_r = pd.DataFrame(safe_r), m=m, start = start, floor = floor, drawdown = drawdown)
    wealth = btr["Wealth"]
    floorhist = btr["floorhist"]
    ...
    var1 = sorted_ret1[int(0.01*n_scenarios)]
...
show_cppi_cir_d(10000, mu = 0.12, sigma1 =0.288, m = 3, floor = 0.8, b = 0.004, rt = 0.004, drawdown = 0.2)
...
for i in range(n_simulations):
    for j in range(i+1, n_simulations):
        if sorted_values_2[i] + sorted_values_2[j] < 2*sorted_values_1[i]:
            count_3 += 1
            if sorted_values_2[i] + sorted_values_2[j] + sorted_values_2[i] < 3*sorted_values_1[i]:
                count_4 += 1
                if sorted_values_2[i] + sorted_values_2[j] + sorted_values_2[i] + sorted_values_2[j] < 4*sorted_values_1[i]:
                    count_5 += 1
...
def run_cppi_cir_d_tc(risky_r, safe_r, m=3, start=1000, floor=0.8, a = 1, b = 0.1/12, rt = 0.1/12, sigma = 0.09, drawdown = 0.1):
    """
    Run a backtest of the CPPI strategy, given a set of returns for the risky asset
    Returns a dictionary containing: Asset Value History, Risk Budget History, Risky Weight History
    """
...
def show_cppi_cir_d_tc(n_scenarios = 50, mu = 0.07, sigma1 = 0.15, m = 3, floor = 0.8, a = 1, b = 0.1/12, rt = 0.1/12, sigma = 0.09, drawdown = 0.1, y_max = 100):
    start = 100
    sim_rets = gbm(n_scenarios = n_scenarios, mu = mu, sigma1 = sigma1, prices = False, steps_per_year = 12)
    risky_r = pd.DataFrame(sim_rets)
    sim_rets1 = run_sim_cir(n_scenarios = n_scenarios, a = a, b = b, rt = rt, sigma = sigma, steps_per_year = 12)
    safe_r = pd.DataFrame(sim_rets1)
    btr = run_cppi_cir_d_tc(risky_r = pd.DataFrame(risky_r), safe_r = pd.DataFrame(safe_r), m=m, start = start, floor = floor, drawdown = drawdown)
    wealth = btr["Wealth"]
    y_max = wealth.values.max()*y_max/100
    terminal_wealth = wealth.iloc[-1]
...
show_cppi_cir_d_tc(10000, mu = 0.12, sigma1 =0.28, m = 3, floor = 0.8, b = 0.004, rt = 0.004, drawdown = 0.2)
